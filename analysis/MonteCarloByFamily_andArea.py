...
#############
dataX = []
with open('../data/phoibleTonesAndHumidity.csv', 'rb') as csvfile:
...
dataX = [x for x in dataX if x[header.index(family_field_name)]!="NA"]
dataX = [x for x in dataX if x[header.index("specH.mean")]!="NA"]
dataX = [x for x in dataX if x[header.index("Tones")]!="NA"]

print "Number of observations",len(dataX)
...
for family_field_name in ["Family"]:
	...
	number_of_tones = [int(x[header.index("Tones")]) for x in data[1:-1]]
